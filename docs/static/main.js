...
var $author$project$Main$Question = F3(
	function (enable, url, image) {
		return {enable: enable, image: image, url: url};
	});
...
var $author$project$Main$FetchErr = function (a) {
	return {$: 'FetchErr', a: a};
};
var $author$project$Main$fromResult = function (res) {
	if (res.$ === 'Ok') {
		var questions = res.a;
		return $author$project$Main$Fetch(questions);
	} else {
		var err = res.a;
		return $author$project$Main$FetchErr(err);
	}
};
...
var $author$project$Main$fetch = function (url) {
	return $elm$http$Http$get(
		{
			expect: A2(
				$elm$http$Http$expectJson,
				$author$project$Main$fromResult,
					...
					A2(
						...
						A2(
							...
							A2(
								...
								$elm$url$Url$Parser$Query$string('questions')),
							_Utils_update(
								url,
								{path: ''}))))));
	});
var $author$project$Main$init = F3(
	function (_v0, url, key) {
		return A2(
			...
			url,
			A2($author$project$Main$Model, _List_Nil, key));
	});
...
var $author$project$Main$update = F2(
	function (msg, model) {
		var _v0 = _Utils_Tuple2(msg, model.questions);
		_v0$6:
		while (true) {
			switch (_v0.a.$) {
				case 'Fetch':
					var questions = _v0.a.a;
					return _Utils_Tuple2(
						model,
						A2(
							$elm$random$Random$generate,
...
var $author$project$Main$ClickNext = {$: 'ClickNext'};
...
var $author$project$Main$viewNext = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
			$elm$html$Html$Attributes$class('f3 m-2'),
			$elm$html$Html$Events$onClick($author$project$Main$ClickNext)
		]),
	_List_fromArray(
		[
			A2(
...
var $author$project$Main$viewQuestion = function (q) {
	return _List_fromArray(
		[
			A2(
			$elm$html$Html$a,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$href(q.url)
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$src(q.image),
...
var $author$project$Main$viewBody = function (model) {
	var question = A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$text('Questions is empty...')
			]),
		A2(
			$elm$core$Maybe$map,
			$author$project$Main$viewQuestion,
			$elm$core$List$head(model.questions)));
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		A2($elm$core$List$cons, $author$project$Main$viewNext, question));
};
